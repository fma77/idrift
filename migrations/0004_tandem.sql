-- Tandem battles against the house get their own boards ('tandem').
--
-- As in 0002, SQLite cannot change a CHECK constraint in place, so the table is
-- rebuilt with the mode list widened and every row copied across unchanged.

CREATE TABLE scores_new (
  id             TEXT    PRIMARY KEY,
  route_id       TEXT    NOT NULL,
  route_version  INTEGER NOT NULL,
  mode           TEXT    NOT NULL CHECK (mode IN ('timeAttack', 'timeAttackPro', 'driftRun', 'tandem')),
